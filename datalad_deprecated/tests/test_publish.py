...
@with_tempfile
@with_tempfile
def test_since_empty_and_unsupported(p1=None, p2):
...
@with_tempfile(mkdir=True)
@with_tempfile(mkdir=True)
def test_publish_simple(origin=None, src_path, dst_path):
...
@with_tempfile(mkdir=True)
@with_tempfile(mkdir=True)
def test_publish_plain_git(origin=None, src_path, dst_path):
...
@with_tempfile(mkdir=True)
@with_tempfile(mkdir=True)
def test_publish_recursive(pristine_origin=None, origin_path, src_path, dst_path, sub1_pub, sub2_pub):
...
@with_tempfile(mkdir=True)
@with_tempfile
def test_publish_with_data(origin=None, src_path, dst_path, sub1_pub, sub2_pub, dst_clone_path):
...
@with_tempfile(mkdir=True)
@with_tempfile(mkdir=True)
def test_publish_gh1691(origin=None, src_path, dst_path):
